...
<body>
    ...
    <section class="main">
        ...
        <div class="main--content">
            ...
            <button class="add" onclick="exportTableToExcel('attendaceTable', '<?php echo htmlspecialchars($unitCode, ENT_QUOTES, 'UTF-8'); ?>_on_<?php echo date('Y-m-d'); ?>','<?php echo htmlspecialchars($coursename, ENT_QUOTES, 'UTF-8'); ?>', '<?php echo htmlspecialchars($unitname, ENT_QUOTES, 'UTF-8'); ?>')">Export Attendance As Excel</button>
...
<script>
    ...
    // Live polling to refresh attendance body every 5s
    (function() {
        ...
        async function poll() {
            try {
                const resp = await fetch(`attendance_data?course=${encodeURIComponent(course)}&unit=${encodeURIComponent(unit)}`);
                if (!resp.ok) return;
                const data = await resp.json();
                if (data && data.status === 'success' && data.html) {
                    const body = document.getElementById('attBody');
                    if (body) body.outerHTML = data.html;
                }
            } catch (e) {
                console.error(e);
            }
